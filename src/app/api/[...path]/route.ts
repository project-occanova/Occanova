...
import {backendReady,hasDatabase,hasEmail,hasStorage,localPreview,publicIntakeEnabled,readOnlyDeployment} from '@/lib/config';
import {rateLimited} from '@/lib/rate-limit';
import {sendEnquiryNotifications,sendResetEmail,sendVerificationEmail} from '@/lib/email';
import {cleanupOrphanedUploads,createDownload,createUpload,deleteUploads,validVendorKey,vendorStorageKeys} from '@/lib/storage';
export const runtime='nodejs';
const fail=(message:string,status=400)=>NextResponse.json({error:message},{status});
export async function GET(req:NextRequest,{params}:{params:Promise<{path:string[]}>}) {
 const route=(await params).path.join('/');
 if(route==='health')return NextResponse.json({ok:true,database:hasDatabase(),email:hasEmail(),storage:hasStorage(),writable:backendReady()&&!readOnlyDeployment(),intake:publicIntakeEnabled()});
