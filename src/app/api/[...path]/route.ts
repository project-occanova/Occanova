...
export async function POST(req:NextRequest,{params}:{params:Promise<{path:string[]}>}) {
 try {
  ...
  if(route==='auth/register') {
   ...
   let delivered=true;try{await sendVerificationEmail(v.email,t);}catch{delivered=false;}
   return NextResponse.json({ok:true,id,message:localPreview()?'Account created. Use the local verification link below.':delivered?'Account created. Check your email to verify your address.':'Account created, but email delivery was delayed. Request a new verification link.',previewUrl:localPreview()?`/verify?token=${t}`:undefined});
  }
  ...
  if(route==='auth/forgot') {const email=z.string().email().transform(x=>x.toLowerCase()).parse(b.email);const t=token();let found=false;await mutate(s=>{const user=s.users.find(u=>u.email===email);if(user){found=true;s.tokens=s.tokens.filter(x=>!(x.userId===user.id&&x.kind==='reset'));s.tokens.push({hash:digest(t),userId:user.id,kind:'reset',expires:Date.now()+1800000});}});if(found)await sendResetEmail(email,t);return NextResponse.json({ok:true,message:'If an account exists, a password reset link has been sent.',previewUrl:found&&localPreview()?`/reset-password?token=${t}`:undefined});}
  if(route==='auth/resend') {const email=z.string().email().transform(x=>x.toLowerCase()).parse(b.email);const t=token();let found=false;await mutate(s=>{const user=s.users.find(u=>u.email===email&&!u.verified);if(user){found=true;s.tokens=s.tokens.filter(x=>!(x.userId===user.id&&x.kind==='verify'));s.tokens.push({hash:digest(t),userId:user.id,kind:'verify',expires:Date.now()+86400000});}});if(found)await sendVerificationEmail(email,t);return NextResponse.json({ok:true,message:'If an unverified account exists, a new verification link has been sent.',previewUrl:found&&localPreview()?`/verify?token=${t}`:undefined});}
