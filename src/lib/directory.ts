...
export function publicVendors(vendors:Vendor[], filter:{category?:string; city?:string; q?:string}={}) {
  return vendors.filter(v=>v.status==='approved' && v.published && (!filter.category || slugify(v.category)===slugify(filter.category)) && (!filter.city || v.locations.some(l=>slugify(l)===slugify(filter.city!))) && (!filter.q || `${v.name} ${v.summary} ${v.category}`.toLowerCase().includes(filter.q.toLowerCase())))
