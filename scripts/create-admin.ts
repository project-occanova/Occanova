...
mutate(s=>{if(s.users.some(x=>x.email===email))throw Error('Account already exists');s.users.push({id:randomUUID(),email,phone:'',passwordHash:hashPassword(password),verified:true,role:'admin'});}).then(()=>console.log('Admin created. Sign in at /login.'));
